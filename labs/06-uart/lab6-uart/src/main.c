...
ISR(TIMER1_OVF_vect)
{
  ...
  uint8_t value;
  char stringDec[8];  // String for converted numbers by itoa()
  char stringHex[8];  // String for converted numbers by itoa()
  char stringBin[8];  // String for converted numbers by itoa()
  char string[8];
    ...
    if (value != '\0') {  // Data available from UART
        ...
        if (value == '1') {
          itoa(no_of_overflows,string, 10);
          uart_puts(string);
        }
    }
